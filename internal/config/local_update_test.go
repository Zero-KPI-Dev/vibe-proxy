...
func TestUpsertLocalProvider(t *testing.T) {
	...
	cfg, err := UpsertLocalProvider(path, LocalProviderInput{ID: "newapi", Type: "openai-compatible", BaseURL: "http://127.0.0.1:3000/v1", APIKeyEnv: "NEW_API_KEY", Models: []string{"deepseek-v4-flash"}, Alias: "vibe-chat", AliasModel: "deepseek-v4-flash", DefaultModel: "vibe-chat"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(string(written), "NEW_API_KEY") || !strings.Contains(string(written), "vibe-chat") {
